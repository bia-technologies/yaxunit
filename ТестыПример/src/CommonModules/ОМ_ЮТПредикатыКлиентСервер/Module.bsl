...
Процедура ПроверитьПредикаты() Экспорт
	...
	Резуьльтат = ЮТПредикатыКлиентСервер.ПроверитьПредикаты(1, ЮТест.Предикат().ИмеетТип("Число").Больше(0).Меньше(2).Равно(1));
	ЮТест.ОжидаетЧто(Резуьльтат).Свойство("Успешно").ЭтоИстина();
...
Процедура ПроверитьСообщениеОбработкиПредиката(ПроверяемоеЗначение, Предикат, Сообщение)
	...
	Описание = СтрШаблон("Проверка предиктата %1 `%2` %3", Предикат.ИмяРеквизита, Предикат.ВидСравнения, Предикат.Значение);
