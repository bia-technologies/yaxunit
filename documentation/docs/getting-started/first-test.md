...
Тесты с использованием YAxUnit пишутся в расширении, это может быть как отдельное расширение, так и расширение движка. Подробнее о плюсах и минусах этих вариантов вы можете почитать в [статье об организации тестов](structure.md).
...
Следующим шагом необходимо создать экспортную процедуру с наименованием `ИсполняемыеСценарии`, это главный метод тестового набора, здесь необходимо будет перечислить тесты, которые содержит модуль. Так как в 1С:Предприятии нельзя в режиме исполнения понять, какие методы содержит модуль (нет рефлексии).
...
Можно еще доработать наш тест, проверить разные варианты сложения, для этого могут помочь параметры теста.
...
Поздравляю с первым реализованным тестом.
Используя [руководство](../features) и имеющиеся тесты, вы сможете дальше изучить их и написать более полезные тесты для своих продуктов.
