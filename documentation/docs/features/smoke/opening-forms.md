...
Реализовано несколько видов тестов открытия форм
...
* `Скопированный` - Открывает форму **скопированного** объекта.
* `СкопированнаяГруппа` - Открывает форму **скопированной группы**.
...
Общие формы:
* `Общая` - Открывает общую форму

### Объекты метаданных

Поддерживается большинство объектов метаданных. Так же доступно тестирование общих форм.
...
которые распространятся на нижние уровни, при этом на нижнем уровне можно их переопределить.
...
```json
{
    "ДымовыеТесты": {
        ...
        "ОткрытиеФорм": {
            ...
            "ОбщиеФормы": false // Отключение тестов общих форм
        }
    }
}
```

Еще одна особенность настроек - для отключения/включения какой-то настройки необязательно создавать объект с реквизитом `Использовать`, можно сразу присвоить булевое значение.
...
```json title="Исключения.json"
{
  ...
  "ДымовыеТесты": {
      ...
      "ОткрытиеФорм": {
          ...
          "ОбщиеФормы": {
            "Использовать": true, // Включение тестов открытия общих форм
            "Исключения": [ // Заполенние списка исключений для общийх форм
                "АвтономнаяРабота*" // Исключение всех общих форм начинающихся на АвтономнаяРабота
            ]
          }
      }
  }
