...
* Реализован достаточный набор проверок значений
  ...
  * [Проверка методов объекта](#проверка-методов-объекта)
...
* [`ВыбрасываетИсключение`](/api/ЮТУтверждения#выбрасываетисключение) - проверят, что указанный метод объекта выбрасывает исключение.  
  
  :::caution
  Часто задается вопрос, можно ли не указывать текст исключения. Ответ, нет, нельзя, вы должны конкретизировать свои ожидания, зафиксировать, какую ошибку ждете, иначе поведение может измениться и тест это не отработает.
  :::
* [`НеВыбрасываетИсключение`](/api/ЮТУтверждения#невыбрасываетисключение) - проверят, что указанный метод объекта не выбрасывает исключение.  
  :::caution
  ...
  Владельца метода, `ЮТест.ОжидаетЧто(Документ)`  
  Имя метода, `.Метод("Записать")`  
  Параметры вызова метода, `.Параметр(РежимЗаписиДокумента.Проведение)`
...
* [`Содержит`](/api/ЮТУтверждения#содержит) - проверяемая коллекция должна содержать элемент, который соответствует переданным условиям
...
Для проверки работы методов объекта есть набор утверждений среди описанных выше ([`ВыбрасываетИсключение`](/api/ЮТУтверждения#выбрасываетисключение) и [`НеВыбрасываетИсключение`](/api/ЮТУтверждения#невыбрасываетисключение)), но для их работы необходимо выполнить предварительные настройки.
