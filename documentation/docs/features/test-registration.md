...
* Настройка обработчиков [событий](events.md)
  * [`Перед`](/api/ЮТТесты#перед) - Указывает, какой метод будет выполняться перед тестом
  * [`После`](/api/ЮТТесты#после) - Указывает, какой метод будет выполняться после тестом
...
То же самое относится и к методам ниже
:::
...
В форме запуска тестов модуль не выводится отдельной строкой, поэтому теги модуля отображаются в колонке «Теги» у каждого его набора.
...
Подробнее об установке параметров Allure можно почитать в отдельной [статье](reports/allure-properties)
