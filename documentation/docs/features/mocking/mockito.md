...
Но перед этим необхдимо перевести `Мокито` в режим **прогона тестов**, для этого используется метод `Прогон`.  
...
Для этого можно воспользоваться методом [`Вызовы`](/api/МокитоПроверки#Вызовы), который возвращает массив с описанием вызовов:
