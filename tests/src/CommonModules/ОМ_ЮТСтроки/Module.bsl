...
Процедура ДополнитьСправа() Экспорт
	
	// Дополнение пробелами (по умолчанию)
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("Тест", 10)).Равно("Тест      ");
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("123", 5)).Равно("123  ");
	
	// Дополнение другим символом
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("123", 5, "0")).Равно("12300");
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("Тест", 10, "-")).Равно("Тест------");
	
	// Строка уже нужной длины - не должна дополняться
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("Тест", 4)).Равно("Тест");
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("123", 3, "0")).Равно("123");
	
	// Строка длиннее ожидаемой - не должна дополняться
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("ТестСтрока", 4)).Равно("ТестСтрока");
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("12345", 3, "0")).Равно("12345");
	
	// Пустая строка
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("", 5)).Равно("     ");
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("", 3, "0")).Равно("000");
	
	// Дополнение до длины 1
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("", 1)).Равно(" ");
	ЮТест.ОжидаетЧто(ЮТСтроки.ДополнитьСправа("", 1, "X")).Равно("X");
	
	// Ошибка при передаче символа длиной не 1
	ЮТест.ОжидаетЧто(ЮТСтроки)
		.Метод("ДополнитьСправа", ЮТКоллекции.ЗначениеВМассиве("Тест", 10, "12"))
		.ВыбрасываетИсключение("Символ должен иметь длину 1 символ");
	ЮТест.ОжидаетЧто(ЮТСтроки)
		.Метод("ДополнитьСправа", ЮТКоллекции.ЗначениеВМассиве("Тест", 10, ""))
		.ВыбрасываетИсключение("Символ должен иметь длину 1 символ");
...
Процедура ЭтоЧисловаяСтрока() Экспорт
	// Корректные числовые строки
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("123")).ЭтоИстина();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("123.45")).ЭтоИстина();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("-123")).ЭтоИстина();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("-123.45")).ЭтоИстина();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("0")).ЭтоИстина();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("0.0")).ЭтоИстина();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока(".5")).ЭтоИстина();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("-.5")).ЭтоИстина();
	
	// Строки с пробелами (должны обрезаться)
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока(" 123 ")).ЭтоИстина();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока(" -123.45 ")).ЭтоИстина();
	
	// Некорректные строки - без цифр
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока(".")).ЭтоЛожь();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("-")).ЭтоЛожь();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("-.")).ЭтоЛожь();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("")).ЭтоЛожь();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("   ")).ЭтоЛожь();
	
	// Некорректные строки - с недопустимыми символами
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("abc")).ЭтоЛожь();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("12a")).ЭтоЛожь();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("12.34.56")).ЭтоЛожь();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("12-34")).ЭтоЛожь();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("--123")).ЭтоЛожь();
	ЮТест.ОжидаетЧто(ЮТСтроки.ЭтоЧисловаяСтрока("12-")).ЭтоЛожь();
КонецПроцедуры
