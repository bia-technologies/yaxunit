...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты
		...
		.ДобавитьТест("МассивСтруктурИзТаблицы")
		.ДобавитьТест("МассивСтруктурВТаблицу")
	;
...
Процедура МассивСтруктурИзТаблицы() Экспорт
	...
	Результат = ЮТКоллекции.МассивСтруктурИзТаблицы(Данные);
	
	ЮТест.ОжидаетЧто(Результат, "Новая таблица")
		.Равно(Новый Массив());
		
	// Пустая таблица с колонками
	Данные = Новый ТаблицаЗначений();
	Данные.Колонки.Добавить("Поле1");
	Данные.Колонки.Добавить("Поле2");
	
	Результат = ЮТКоллекции.МассивСтруктурИзТаблицы(Данные);
	
	ЮТест.ОжидаетЧто(Результат, "Пустая таблица с колонками")
		.Равно(Новый Массив());
	...
	НоваяСтрока = Данные.Добавить();  НоваяСтрока.Поле1 = 10;   НоваяСтрока.Поле2 = "20";
	НоваяСтрока = Данные.Добавить();  НоваяСтрока.Поле1 = Ложь; НоваяСтрока.Поле2 = '2026-05-01';
	НоваяСтрока = Данные.Добавить();  НоваяСтрока.Поле1 = Новый Структура("Ключ1, Ключ2", 1, 2);
	
	Результат = ЮТКоллекции.МассивСтруктурИзТаблицы(Данные);
	
	ЮТест.ОжидаетЧто(Результат, "Заполненая таблица")
		.ИмеетТип(Тип("Массив"))
		.ИмеетДлину(3)
		.КаждыйЭлементСоответствуетПредикату(ЮТест.Предикат()
			.ИмеетТип(Тип("Структура"))
			.ИмеетДлину(2)
			.ИмеетСвойство("Поле1")
			.ИмеетСвойство("Поле2")
		)
		.Свойство("[0].Поле1").Равно(10)
		.Свойство("[0].Поле2").Равно("20")
		.Свойство("[1].Поле1").Равно(Ложь)
		.Свойство("[1].Поле2").Равно('2026-05-01')
		.Свойство("[2].Поле1").ИмеетТип(Тип("Структура"))
		.Свойство("[2].Поле1.Ключ1").Равно(1)
		.Свойство("[2].Поле1.Ключ2").Равно(2)
		.Свойство("[2].Поле2").ЭтоНеопределено()
	;
	
	#Иначе
		
	ЮТест.Пропустить("Не доступно на клиенте");
		
	#КонецЕсли
		
КонецПроцедуры
	
Процедура МассивСтруктурВТаблицу() Экспорт
	...
	Результат = ЮТКоллекции.МассивСтруктурВТаблицу(Данные);
	
	ЮТест.ОжидаетЧто(Результат, "Пустой массив")
		.Равно(Новый ТаблицаЗначений());
		
	// Массив с пустыми структурами
	Данные = Новый Массив();
	...
	ЮТест.ОжидаетЧто(Результат, "Массив с пустыми структурами")
		.ИмеетТип(Тип("ТаблицаЗначений"))
		.ИмеетДлину(2)
		.Свойство("Колонки").ИмеетДлину(0)
	;
	
	// Массив одинаковых структур
	Данные = Новый Массив();
	Данные.Добавить(Новый Структура("Поле1, Поле2", 10, "20"));
	Данные.Добавить(Новый Структура("Поле1, Поле2", Ложь, '2026-05-01'));
	Данные.Добавить(Новый Структура("Поле1, Поле2", Новый Структура("Ключ1, Ключ2", 1, 2)));
	
	Результат = ЮТКоллекции.МассивСтруктурВТаблицу(Данные);
	
	ЮТест.ОжидаетЧто(Результат, "Массив одинаковых структур")
		.ИмеетТип(Тип("ТаблицаЗначений"))
		.ИмеетДлину(3)
		.Свойство("[0].Поле1").Равно(10)
		.Свойство("[0].Поле2").Равно("20")
	...
	// Массив различных структур
	Данные = Новый Массив();
	Данные.Добавить(Новый Структура("Поле1, Поле2", 10, "20"));
	Данные.Добавить(Новый Структура("Поле2, Поле3", Ложь, '2026-05-01'));
	Данные.Добавить(Новый Структура("Поле4", Новый Структура("Ключ1, Ключ2", 1, 2)));
	
	Результат = ЮТКоллекции.МассивСтруктурВТаблицу(Данные);
	
	ЮТест.ОжидаетЧто(Результат, "Массив различных структур")
		.ИмеетТип(Тип("ТаблицаЗначений"))
		.ИмеетДлину(3)
		.Свойство("Колонки").ИмеетДлину(4)
		.Свойство("[0].Поле1").Равно(10)
		.Свойство("[0].Поле2").Равно("20")
		.Свойство("[0].Поле3").ЭтоНеопределено()
		.Свойство("[0].Поле4").ЭтоНеопределено()
		.Свойство("[1].Поле1").ЭтоНеопределено()
		.Свойство("[1].Поле2").Равно(Ложь)
		.Свойство("[1].Поле3").Равно('2026-05-01')
		.Свойство("[1].Поле4").ЭтоНеопределено()
		.Свойство("[2].Поле1").ЭтоНеопределено()
		.Свойство("[2].Поле2").ЭтоНеопределено()
		.Свойство("[2].Поле3").ЭтоНеопределено()
		.Свойство("[2].Поле4").ИмеетТип(Тип("Структура"))
		.Свойство("[2].Поле4.Ключ1").Равно(1)
		.Свойство("[2].Поле4.Ключ2").Равно(2)
	;
