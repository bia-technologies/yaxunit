...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты.УдалениеТестовыхДанных()
		...
		.ДобавитьТест("СодержитТолькоОднуЗапись")
		.ДобавитьТест("СодержитНесколькоЗаписей")
...
Процедура СодержитТолькоОднуЗапись() Экспорт
	
	Поставщик = ЮТест.Данные().СоздатьЭлемент("Справочники.Контрагенты");
	ИмяТаблицы = "Справочник.Товары";
	Предикат = ЮТест.Предикат().Реквизит("Поставщик").Равно(Поставщик);
	ОжидаемоеОписаниеОшибки = "Ожидали, что проверяемая таблица `Справочник.Товары` содержит только одну запись";
	
	Попытка
		ЮТест.ОжидаетЧтоТаблицаБазы(ИмяТаблицы).СодержитТолькоОднуЗапись(Предикат);
	Исключение
		Ошибка = ИнформацияОбОшибке();
	КонецПопытки;
	
	ПроверитьОшибкуУтверждения(Неопределено, Ошибка, ОжидаемоеОписаниеОшибки);
...
Процедура СодержитНесколькоЗаписей() Экспорт
	
	Поставщик = ЮТест.Данные().СоздатьЭлемент("Справочники.Контрагенты");
	ИмяТаблицы = "Справочник.Товары";
	Предикат = ЮТест.Предикат().Реквизит("Поставщик").Равно(Поставщик);
	ОжидаемоеКоличествоЗаписей = 2;
	ОжидаемоеОписаниеОшибки = "Ожидали, что проверяемая таблица `Справочник.Товары` содержит несколько записей (2)";
	
	Попытка
		ЮТест.ОжидаетЧтоТаблицаБазы(ИмяТаблицы).СодержитНесколькоЗаписей(ОжидаемоеКоличествоЗаписей, Предикат);
	Исключение
		Ошибка = ИнформацияОбОшибке();
	КонецПопытки;
