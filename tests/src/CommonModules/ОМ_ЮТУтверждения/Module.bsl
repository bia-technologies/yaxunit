...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты
		...
		.ДобавитьТест("Равно")
		.ДобавитьТест("Равно_ТекстОшибки")
		.ДобавитьСерверныйТест("Равно_СравнениеСложныхОбъектов")
		...
		.ДобавитьТест("ИмеетДлину_ТекстОшибки")
		.ДобавитьТест("НеИмеетДлину")
		.ДобавитьТест("НеИмеетДлину_ТекстОшибки")
...
Процедура Свойство() Экспорт
	...
	ПроверитьОшибкуУтверждения(
		Ошибка,
		"Ожидали, что проверяемое значение `Массив` содержит значение по индексу `0`, которое равно `2`, но это не так"
	);
...
Процедура Равно_ТекстОшибки() Экспорт
	...
	ОжидаемоеСообщение = СтрШаблон("Ожидали, что проверяемое значение `1` равно `<Пустое значение, Тип: %1>`, но это не так", Тип("Неопределено"));
	ПроверитьОшибкуУтверждения(Ошибка, ОжидаемоеСообщение);
...
Процедура НеРавно() Экспорт
	...
	ПредставлениеЗначения = СтрШаблон("<Пустое значение, Тип: %1>", Тип("Неопределено"));
	ПроверитьОшибкуУтверждения(Ошибка, СтрШаблон("Ожидали, что проверяемое значение `%1` не равно `%1`, но это не так", ПредставлениеЗначения));
...
Процедура Заполнено() Экспорт
	...
	ПредставлениеПустогоЗначения = СтрШаблон("<Пустое значение, Тип: %1>", Тип("Неопределено"));
	ПроверитьОшибкуУтверждения(Ошибка, СтрШаблон("Ожидали, что проверяемое значение `%1` является заполненным, но это не так", ПредставлениеПустогоЗначения));
	...
	ОжидаемоеСообщение = ЮТСтроки.ЛокализованноеСообщениеПлатформы(
		"ru ='Проверка мутабельных значений на заполненность'; en='Mutable values filling check is unavailable'");
...
Процедура НеЗаполнено() Экспорт
	...
	Исключение
		Ошибка = ИнформацияОбОшибке();
	КонецПопытки;
	ОжидаемоеСообщение = ?(ЮТОкружение.ИспользуетсяАнглийскаяЛокальПлатформы(),
...
Процедура Существует() Экспорт
	...
	ПредставлениеЗначение = СтрШаблон("<Пустое значение, Тип: %1>", Тип("Неопределено"));
	ПроверитьОшибкуУтверждения(Ошибка, СтрШаблон("Ожидали, что проверяемое значение `%1` существует, но это не так", ПредставлениеЗначение));
	ПроверитьДанныеОшибкиСравнения(Неопределено, Неопределено);
...
Процедура НеСуществует() Экспорт
	...
	ПредставлениеЗначение = СтрШаблон("<Пустое значение, Тип: %1>", Тип("Строка"));
	ПроверитьОшибкуУтверждения(Ошибка,
		СтрШаблон("Пустая строка: ожидали, что проверяемое значение `%1` не существует, но это не так", ПредставлениеЗначение));
...
Процедура ЭтоНеопределено() Экспорт
	...
	ПредставлениеЗначение = СтрШаблон("<Пустое значение, Тип: %1>", Тип("Строка"));
	ПроверитьОшибкуУтверждения(Ошибка,
		СтрШаблон("Пустая строка: ожидали, что проверяемое значение `%1` является неопределено, но это не так", ПредставлениеЗначение));
...
Процедура ЭтоНеНеопределено() Экспорт
	...
	ПредставлениеЗначение = СтрШаблон("<Пустое значение, Тип: %1>", Тип("Неопределено"));
	ПроверитьОшибкуУтверждения(Ошибка,
		СтрШаблон("Ожидали, что проверяемое значение `%1` не является неопределено, но это не так", ПредставлениеЗначение));
	ПроверитьДанныеОшибкиСравнения(Неопределено, Неопределено);
...
Процедура ЭтоNull() Экспорт
	...
	ПредставлениеЗначение = СтрШаблон("<Пустое значение, Тип: %1>", Тип("Неопределено"));
	ПроверитьОшибкуУтверждения(Ошибка, СтрШаблон("Ожидали, что проверяемое значение `%1` является null, но это не так", ПредставлениеЗначение));
...
Процедура ИмеетТип() Экспорт
	...
	ОжидаемоеСообщение = ?(ЮТОкружение.ИспользуетсяАнглийскаяЛокальПлатформы(), "Type is not defined", "Тип не определен");
	ПроверитьОшибкуИсполнения(Ошибка, ОжидаемоеСообщение);
...
Процедура Содержит() Экспорт
	...
	Исключение
		Ошибка = ИнформацияОбОшибке();
	КонецПопытки;
	ПроверитьОшибкуИсполнения(Ошибка, СтрШаблон("Тип проверяемого значения `%1` не может быть проверен на содержание ожидаемого значения",
												Тип("Неопределено")));
...
Процедура ИмеетДлину_ТекстОшибки() Экспорт
	...
	Исключение
		Ошибка = ИнформацияОбОшибке();
	КонецПопытки;
	ПроверитьОшибкуИсполнения(Ошибка, СтрШаблон("Дата: Тип проверяемого значения `%1` не может быть проверен на длинну значения", Тип("Дата")));
...
Процедура НеИмеетДлину_ТекстОшибки() Экспорт
...
Процедура ВыбрасываетИсключение() Экспорт
	...
	МетодНеОбнаружен = ?(ЮТОкружение.ИспользуетсяАнглийскаяЛокальПлатформы(), "Object method not found (НесуществующийМетод)", "Метод объекта не обнаружен (НесуществующийМетод)");
	МногоПараметров = ?(ЮТОкружение.ИспользуетсяАнглийскаяЛокальПлатформы(), "Too many actual parameters", "Слишком много фактических параметров");
	МалоПараметров = ?(ЮТОкружение.ИспользуетсяАнглийскаяЛокальПлатформы(), "Not enough actual parameters", "Недостаточно фактических параметров");
...
Процедура УказаниеПараметровПоУмолчанию() Экспорт
	
	ЮТУтверждения.Что(ОМ_ЮТУтверждения)
		.Метод("МетодСоЗначениеПоУмолчанию", ЮТМетоды.МассивПараметров(, "Исключение"))
		.НеВыбрасываетИсключение("Значение отличается от значения по умолчанию")
		.НеВыбрасываетИсключение()
	;
...
Процедура Равно_СравнениеСложныхОбъектов() Экспорт
...
Процедура НачинаетсяС() Экспорт
	...
	ПредставлениеЗначение = СтрШаблон("<Пустое значение, Тип: %1>", Тип("Строка"));
	ПроверитьОшибкуУтверждения(Ошибка,
		СтрШаблон("Ожидали, что проверяемое значение `%1` начинается с `ризоватовский`, но это не так", ПредставлениеЗначение));
...
Процедура ЗаканчиваетсяНа() Экспорт
	...
	ПредставлениеЗначение = СтрШаблон("<Пустое значение, Тип: %1>", Тип("Строка"));
	ПроверитьОшибкуУтверждения(Ошибка,
		СтрШаблон("Ожидали, что проверяемое значение `%1` заканчивается на `сельсовет`, но это не так", ПредставлениеЗначение));
...
Функция МетодСоЗначениеПоУмолчанию(Значение = 999, ОбязательноеЗначение) Экспорт
	...
	Если Значение <> 999 Тогда
		ВызватьИсключение "Значение отличается от значения по умолчанию";
	КонецЕсли;
	
	Возврат ОбязательноеЗначение;
