...
Процедура МассивПараметров() Экспорт
	
	Результат = Мокито.МассивПараметров();
	ЮТест.ОжидаетЧто(Результат, "Получение пустого массива параметров")
		.ИмеетТип("Массив").ИмеетДлину(0);
	
	Результат = Мокито.МассивПараметров(42);
	ЮТест.ОжидаетЧто(Результат, "Получение массива параметров")
		.ИмеетТип("Массив").ИмеетДлину(1)
		.Свойство("[0]").Равно(42);
	
	Результат = Мокито.МассивПараметров( , 42);
	ЮТест.ОжидаетЧто(Результат, "Получение массива параметров с пропущенным значением")
		.ИмеетТип("Массив").ИмеетДлину(2)
		.Свойство("[0]").Равно(Мокито.ЛюбойПараметр())
		.Свойство("[1]").Равно(42);
