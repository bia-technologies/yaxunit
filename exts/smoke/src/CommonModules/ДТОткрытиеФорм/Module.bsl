...
Процедура ИсполняемыеСценарии() Экспорт
	...
	Для Каждого КоллекцияМетаданных Из ДеревоТестов Цикл
		...
		ЮТТесты.ДобавитьТестовыйНабор(КоллекцияМетаданных.ИмяКоллекции)
			.Настроить(ЮТАлюр.Suite(КоллекцияМетаданных.ИмяКоллекции));
...
Процедура ЗарегистрироватьТест(ОписаниеОбъекта, ТипТеста, Представление)
	...
	ПредставлениеОбъекта = СтрШаблон("%1.%2", ОписаниеОбъекта.Тип, ОписаниеОбъекта.Имя);
	ЮТТесты.ДобавитьКлиентскийТест(ИмяМетода, ПредставлениеТеста, ЮТКоллекции.ЗначениеВМассиве(ТипТеста))
		.Настроить(ЮТАлюр
			...
			.SubSuite(ОписаниеОбъекта.Имя)
			.Package("Smoke.ДТОткрытиеФорм." + ПредставлениеОбъекта))
...
Процедура ОткрытьФорму_СуществующаяГруппа(ОписаниеОбъекта) Экспорт
	...
	ОткрытьЗакрытьФорму(ОписаниеОбъекта, "ФормаГруппы", Ключ);
...
Процедура ОткрытьФорму_Скопированный(ОписаниеОбъекта) Экспорт
	...
	ОткрытьЗакрытьФорму(ОписаниеОбъекта, "ФормаОбъекта", Ключ, "ЗначениеКопирования");
...
Процедура ОткрытьФорму_Общая(ОписаниеОбъекта) Экспорт

	ОткрытьЗакрытьФорму(ОписаниеОбъекта, "ОбщаяФорма");
	
КонецПроцедуры

Процедура ОткрытьЗакрытьФорму(ОбъектМетаданных, ИмяФормы, Ключ = Неопределено)
	
	Если ОбъектМетаданных.Тип = "ОбщаяФорма" Тогда
		ПолноеИмяФормы = СтрШаблон("%1.%2", ОбъектМетаданных.Тип, ОбъектМетаданных.Имя);	
	Иначе
	...
	ЮТЛогирование.Отладка(СтрШаблон("	%1(ссылка): %2",ИмяРеквизитаКлюча, Ключ));
	
	Параметры = Новый Структура(ИмяРеквизитаКлюча, Ключ);
