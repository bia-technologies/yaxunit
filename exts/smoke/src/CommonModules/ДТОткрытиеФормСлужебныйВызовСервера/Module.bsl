...
Функция ТипыТестов() Экспорт
	...
	ЗаполнитьОбщимиФормами(ПараметрыТестов, ОписанияКоллекций);  
...
Функция ПараметрыЗагрузки()
	...
	ДоступныОбычныеФормы = Окружение.ОбычноеПриложение ИЛИ (Метаданные.ИспользоватьОбычныеФормыВУправляемомПриложении И Окружение.ТолстыйКлиент);
	ДоступныУправляемыеФормы = НЕ Окружение.ОбычноеПриложение ИЛИ Метаданные.ИспользоватьУправляемыеФормыВОбычномПриложении;
...
Процедура ЗаполнитьОбщимиФормами(ПараметрыТестов, ОписанияКоллекций)
	...
	ОписанияКоллекцийОбщихФорм = ОбработатьКоллекциюОбщихФорм(ИмяКоллекции, Настройка);
	
	Если ЗначениеЗаполнено(ОписанияКоллекцийОбщихФорм) Тогда
		Описание = Новый Структура("ИмяКоллекции, Тип, Объекты", ИмяКоллекции, Тип, ОписанияКоллекцийОбщихФорм);
		ОписанияКоллекций.Добавить(Описание);
	КонецЕсли; 
...
Функция ОбработатьКоллекциюОбщихФорм(ИмяКоллекции, НастройкаКоллекции)

    Коллекция = Метаданные[ИмяКоллекции];
	ОписанияОбъектов = Новый Массив;
	
	Для Каждого ОбщаяФорма Из Коллекция Цикл
		
		Если ОбщаяФорма.Имя = "ВходВПрограммуЗапрещен" Тогда
			Продолжить;	
		КонецЕсли;  
		
		Если НЕ ПравоДоступа("Просмотр", ОбщаяФорма)
			ИЛИ ЮТДымовыеТестыСлужебныйКлиентСервер.ПодходитПодНаборыИсключений(ОбщаяФорма.Имя, НастройкаКоллекции.НаборыИсключений) Тогда
			Продолжить;
		КонецЕсли;
		
		Настройка = ПереопределеннаяНастройка(НастройкаКоллекции, ОбщаяФорма.Имя);
		
		Если НЕ Настройка.Использовать Тогда
			Продолжить;
		КонецЕсли;
			
		ОписаниеОбъекта = Новый Структура("Имя, ТипыТестов", ОбщаяФорма.Имя, Новый Массив);
		ОписаниеОбъекта.ТипыТестов.Добавить("Общая");    
