...
Функция ВыполнитьМетод(ПолноеИмяМетода, Параметры = Неопределено, Объект = Неопределено) Экспорт
	...
	Если Объект <> Неопределено Тогда
		Возврат ЮТМетодыСлужебный.ВыполнитьМетодОбъекта(Объект, ПолноеИмяМетода, Параметры);
	Иначе
		Возврат ЮТМетодыСлужебный.ВыполнитьМетодКонфигурации(ПолноеИмяМетода, , Параметры);
	КонецЕсли;
