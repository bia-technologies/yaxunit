...
//
Функция СформироватьКодПоСсылкамНаОбъекты(СсылкиНаОбъекты, НастройкиГенерации) Экспорт
...
Процедура ШагУстановитьРеквизит(Контекст, Имя, Значение)
	...
	Если НЕ НастройкиГенерации.ПропускатьПустыеЗначения ИЛИ ЗначениеЗаполнено(Значение) Тогда
		...
		ИначеЕсли Выражение = Неопределено И ЮТТипыДанныхСлужебный.ЭтоСсылочныйТип(ТипЗнч(Значение)) И НЕ Контекст.НастройкиГенерации.ИспользоватьПоиск Тогда
			Конструктор
...
//
// Возвращаемое значение:
//  Строка - строковое представление типов, разделенных запятыми.
//
...
// Преобразует тип значения в строковое представление с учетом квалификаторов.
//
// Параметры:
//  ТипЗначения - Тип - тип значения для преобразования.
//  ОписаниеТипов - ОписаниеТипов, Неопределено - описание типов для получения квалификаторов.
//
// Возвращаемое значение:
//  Строка - строковое представление типа с квалификаторами (если применимо).
//
...
Функция ВыраженияПоСсылкам(СчетчикСсылок, ПараметрыГенерацииКода)
	...
	Для Каждого ЭлементПоОбъекту Из СчетчикСсылок Цикл
		Если ЭлементПоОбъекту.Значение > 1 ИЛИ ПараметрыГенерацииКода.ИспользоватьПоиск Тогда // В других случаях используется фикция значений
			Выражения[ЭлементПоОбъекту.Ключ]  = СсылочноеЗначениеВСтроку(ЭлементПоОбъекту.Ключ, ПараметрыГенерацииКода);
		КонецЕсли;
	КонецЦикла;
...
Функция УпорядочитьПеременные(ПодготовленныеДанные)
	...
	Переменные.Сортировать("Порядок УБЫВ, НомерСтроки, Тип, ИмяПеременной");
...
//  // Результат: "Истина"
//
Функция ЗначениеВСтроку(Значение, ПараметрыГенерацииКода)
...
Функция ВыражениеПримитивногоЗначения(Значение, ТипЗначения)
	...
	ИначеЕсли ТипЗначения = Тип("ОписаниеТипов") Тогда
		...
		ЭкранированнаяСтрока = ЭкранированиеСтроки(СтрокаТипов); // Замена одной кавычки на две
...
Функция ВыражениеПоискаСсылочногоЗначения(Значение, ОписаниеОбъекта)
	...
	Иначе
		...
		Если ЮТМетаданные.ЭтоПланСчетов(ОписаниеОбъекта) И ОписаниеОбъекта.Реквизиты.Свойство("Код") Тогда
...
Функция ВыражениеСозданияСсылочногоЗначения(Значение, ОписаниеОбъекта)
	...
	Если ЮТМетаданные.ЭтоДокумент(ОписаниеОбъекта) Тогда
		Результат = СтрШаблон("ЮТест.Данные().СоздатьДокумент(""%1"");", ИмяМенеджера, Строка(Значение));
