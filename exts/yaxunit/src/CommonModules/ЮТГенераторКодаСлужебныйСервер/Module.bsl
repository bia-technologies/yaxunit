...
//  ДопИнфоКомментарий - Строка, Неопределено - переменная для возврата дополнительной информации в виде комментария.
//
// Возвращаемое значение:
...
Функция ПодготовитьДанныеПоСсылкам(СсылкиНаОбъекты, ПараметрыГенерацииКода)
	...
	Результат.Вставить("СсылочныеЗначения", Новый Соответствие);
	...
	СсылочныеЗначения = Новый Соответствие;
	Для Каждого ЭлементПоОбъекту Из СсылочныеЗначенияПоОбъектам Цикл
		Для Каждого СсылочныеЗначение Из ЭлементПоОбъекту.Значение Цикл
			ОписаниеСсылки = СсылочныеЗначения.Получить(СсылочныеЗначение.Ключ);
			Если ОписаниеСсылки <> Неопределено Тогда
				ОписаниеСсылки.Количество = ОписаниеСсылки.Количество + СсылочныеЗначение.Значение;
			Иначе
				СсылочныеЗначения.Вставить(СсылочныеЗначение.Ключ, Новый Структура("Количество, Выражение", СсылочныеЗначение.Значение));
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	// Преобразуем ссылки в строковое представление
	ПреобразоватьСсылкиВСтроку(СсылочныеЗначения);
	
	// Формируем имена переменных для повторяющихся ссылок
	Результат.СсылочныеЗначения = СсылочныеЗначения;
	Результат.ИменаПеременных = СформироватьИменаПеременныхДляСсылок(СсылочныеЗначения);
...
Процедура ГенерацияПеременных(Конструктор, ПодготовленныеДанные)
	...
	Для Каждого ЭлементПеременной Из ПодготовленныеДанные.ИменаПеременных Цикл
		...
		ПараметрыСсылки = ПодготовленныеДанные.СсылочныеЗначения[Ссылка];
		
		Если ПараметрыСсылки <> Неопределено Тогда
			
			Конструктор.НоваяСтрокаПоШаблону("%1 = %2;", ИмяПеременной, ПараметрыСсылки.Выражение).ПереносСтроки();
			ПараметрыСсылки.Выражение = ИмяПеременной;
...
Функция СформироватьКодПоСсылкамНаОбъекты(СсылкиНаОбъекты, ПараметрыГенерацииКода) Экспорт
	...
	Для Каждого Ссылка Из ПодготовленныеДанные.УпорядоченныеСсылки Цикл
		...
		СформироватьКодПоСсылке(Конструктор, Объект, ОписаниеОбъекта, ПараметрыГенерацииКода, ПодготовленныеДанные);
...
//  ИменаПеременныхДляСсылок - Соответствие - соответствие ссылок на имена переменных для замены повторяющихся ссылок.
//
// Пример:
...
//
Процедура СформироватьКодПоСсылке(Конструктор, Объект, ОписаниеОбъекта, НастройкиФормирования, ИменаПеременныхДляСсылок = Неопределено) Экспорт
	...
	ШагУстановкаРеквизитов(Конструктор, Объект, ОписаниеОбъекта.Реквизиты, ИменаПеременныхДляСсылок);
	...
	Для Каждого Элемент Из ОписаниеОбъекта.ТабличныеЧасти Цикл
		ШагЗаполненияТабличнойЧасти(Конструктор, Элемент.Ключ, Объект[Элемент.Ключ], Элемент.Значение, ИменаПеременныхДляСсылок);
	КонецЦикла;
...
Процедура ШагЗаполненияТабличнойЧасти(Конструктор, ИмяТабличнойЧасти, Таблица, СтруктураРеквизитовТабличнойЧасти, ИменаПеременныхДляСсылок = Неопределено)
	
	Если ЗначениеЗаполнено(Таблица) Тогда
		Конструктор.НоваяСтрокаПоШаблону(".ТабличнаяЧасть(""%1"")", ИмяТабличнойЧасти);
		ШагЗаполненияТаблицы(Конструктор, Таблица, ЮТКоллекции.ВыгрузитьЗначения(СтруктураРеквизитовТабличнойЧасти, "Значение"), ИменаПеременныхДляСсылок);
	КонецЕсли;
	
КонецПроцедуры

Процедура ШагЗаполненияТаблицы(Конструктор, Таблица, Знач Колонки = Неопределено, ИменаПеременныхДляСсылок = Неопределено)
	...
	Для Каждого СтрокаДвижения Из Таблица Цикл
		...
		Для Каждого Колонка Из Колонки Цикл
			...
			Если ЗначениеЗаполнено(Значение) Тогда
				СтроковоеПредставление = ЗначениеВСтроку(Значение);
...
Процедура ШагУстановкаРеквизитов(Конструктор, Объект, Реквизиты, ИменаПеременныхДляСсылок = Неопределено)
...
//
// Параметры:
//  СсылкиНаОбъекты - Массив - массив ссылок на объекты для анализа.
//
// Возвращаемое значение:
...
//
// Параметры:
//  СсылочныеЗначения - Соответствие из Структура - собранные ссылочные значения
Процедура ПреобразоватьСсылкиВСтроку(СсылочныеЗначения)
	
	Для Каждого ЭлементСсылки Из СсылочныеЗначения Цикл
		
		ЭлементСсылки.Значение.Выражение = ЗначениеВСтроку(ЭлементСсылки.Ключ);
...
//
// Параметры:
//  СсылочныеЗначения - Соответствие - собранные ссылочные значения
//
// Возвращаемое значение:
//  Соответствие - соответствие имен переменных для ссылок:
//    * Ключ - Произвольный - ссылочное значение.
//    * Значение - Строка - имя переменной для данной ссылки.
//
Функция СформироватьИменаПеременныхДляСсылок(СсылочныеЗначения)
	...
	Для Каждого ЭлементСсылки Из СсылочныеЗначения Цикл
		...
		Если ЭлементСсылки.Значение.Количество > 1 Тогда
			ВсеСсылкиСКоличеством.Вставить(ЭлементСсылки.Ключ, Истина);
		КонецЕсли;
		
	КонецЦикла;
	
	// Для ссылок с количеством > 1 формируем имена переменных
	...
	Для Каждого ЭлементСсылки Из ВсеСсылкиСКоличеством Цикл
		...
		Если ПустаяСтрока(БазовоеИмя) ИЛИ СтрНачинаетсяС(БазовоеИмя, "_") Тогда
			ИмяПеременной = ЮТМетаданные.ОписаниеОбъектаМетаданных(Ссылка).Имя + БазовоеИмя;
		Иначе
			ИмяПеременной = БазовоеИмя;
		КонецЕсли;
		
		// Формируем уникальное имя с учетом дублей
		Суффикс = 0;
		
		Пока ИспользованныеИмена.Получить(ИмяПеременной) <> Неопределено Цикл
			Суффикс = Суффикс + 1;
			ИмяПеременной = БазовоеИмя + Строка(Суффикс);
		КонецЦикла;
		
		ИменаПеременных.Вставить(ИмяПеременной, Ссылка);
		ИспользованныеИмена.Вставить(ИмяПеременной, Истина);
