...
Процедура ОбработкаГлобальнойОшибки(ИнформацияОбОшибке, ТребуетсяЗавершениеСеанса, СтандартнаяОбработка) Экспорт
	...
	Если ТекущийЭтап = ЮТФабрика.ЭтапыПрогона().ЗагрузкаТестов Тогда
		...
		ПодключитьОбработчикОжидания("ЮТВосстановлениеРаботыПослеГлобальнойОшибки", 0.1, Истина); // Разрыв стека на случай еще одной ошибки
	КонецЕсли;
...
Процедура СохранитьСостояниеТекущегоЭтапаИсполнения(НовоеСостояние) Экспорт
	...
	ТекущееСостояние = ТекущееСостояниеИсполненияТестов();
	Если ТекущееСостояние.Состояние = Неопределено Тогда
		ТекущееСостояние.Состояние = НовоеСостояние;
	Иначе
		ЮТКоллекции.ДополнитьСтруктуру(ТекущееСостояние.Состояние, НовоеСостояние);
	КонецЕсли;
...
Функция ЗначениеСостоянияТекущегоЭтапаИсполнения(ИмяРеквизита) Экспорт
	...
	Иначе
		Возврат ЮТКоллекции.ЗначениеСтруктуры(ТекущееСостояние.Состояние, ИмяРеквизита);
	КонецЕсли;
