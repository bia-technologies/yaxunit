...
Процедура ОбработчикЗагрузитьЗарегистрированныеТесты(_, ДополнительныеПараметры) Экспорт
	...
	УстановитьТекущийЭтапИсполненияТестов(ЮТФабрика.ЭтапыПрогона().ЗагрузкаТестов);
...
Процедура ОбработчикРазрешитьЗависимости(_, ДополнительныеПараметры) Экспорт
	...
	УстановитьТекущийЭтапИсполненияТестов(ЮТФабрика.ЭтапыПрогона().РазрешениеЗависимостей);
	Обработчик = ЮТАсинхроннаяОбработкаСлужебныйКлиент.СледующийОбработчик(ДополнительныеПараметры);
...
Процедура ОбработчикВыполнитьТестирование(_, ДополнительныеПараметры) Экспорт
	...
	УстановитьТекущийЭтапИсполненияТестов(ЮТФабрика.ЭтапыПрогона().ПрогонТестов);
...
Процедура ОбработчикСохранитьОтчет(_, ДополнительныеПараметры) Экспорт
	...
	УстановитьТекущийЭтапИсполненияТестов(ЮТФабрика.ЭтапыПрогона().ФормированиеОтчета);
...
Процедура УстановитьТекущийЭтапИсполненияТестов(ИмяЭтапа)
	
	ЮТКонтекстСлужебный.УстановитьТекущийЭтапПрогона(ИмяЭтапа);
...
Процедура ОбработкаГлобальнойОшибки(ИнформацияОбОшибке, ТребуетсяЗавершениеСеанса, СтандартнаяОбработка) Экспорт
	...
	Иначе
		ЮТОбщий.СообщитьПользователю(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецЕсли;
	
	ТекущийЭтап = ЮТКонтекстСлужебный.ТекущийЭтапПрогона();
	
	//@skip-check undefined-variable
	Если ТекущийЭтап = Неопределено Или ЮТТекущееСостояниеИсполненияТестов = Неопределено
		Или ЮТТекущееСостояниеИсполненияТестов.Обработчик = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	//@skip-check undefined-variable
	Обработчик = ЮТТекущееСостояниеИсполненияТестов.Обработчик;
	
	Если ТекущийЭтап = ЮТФабрика.ЭтапыПрогона().ЗагрузкаТестов Тогда
		СтандартнаяОбработка = Ложь;
		//@skip-check undefined-variable
		ЮТАсинхроннаяОбработкаСлужебныйКлиент.ВызватьОбработчик(Обработчик, ЮТТекущееСостояниеИсполненияТестов.Результат);
	КонецЕсли;
	
КонецПроцедуры

Процедура СохранитьСостояниеТекущегоЭтапаИсполнения(НовоеСостояние) Экспорт
	
	//@skip-check undefined-variable
	Если ЮТТекущееСостояниеИсполненияТестов.Состояние = Неопределено Тогда
		ЮТТекущееСостояниеИсполненияТестов.Состояние = НовоеСостояние;
	Иначе
		ЮТКоллекции.ДополнитьСтруктуру(ЮТТекущееСостояниеИсполненияТестов.Состояние, НовоеСостояние);
	КонецЕсли;
	
КонецПроцедуры

Функция ЗначениеСостоянияТекущегоЭтапаИсполнения(ИмяРеквизита) Экспорт
	
	//@skip-check undefined-variable
	Если ЮТТекущееСостояниеИсполненияТестов = Неопределено Тогда
		ВызватьИсключение "Текущее состояние исполнения тестов не инициализированно";
	Иначе
		Возврат ЮТКоллекции.ЗначениеСтруктуры(ЮТТекущееСостояниеИсполненияТестов.Состояние, ИмяРеквизита);
	КонецЕсли;
