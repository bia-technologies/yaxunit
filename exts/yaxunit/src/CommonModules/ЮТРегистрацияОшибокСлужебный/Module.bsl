...
Процедура ЗарегистрироватьОшибкуСобытияИсполнения(ИмяСобытия, ОписаниеСобытия, Ошибка) Экспорт
	...
	// Обработка пропуска теста для событий теста.
	Если ОписаниеСобытия.Тест <> Неопределено Тогда
		ТипОшибки = ТипОшибки(Ошибка, ОписаниеСобытия.Тест.ПолноеИмяМетода);
		Если ТипОшибки = ЮТФабрикаСлужебный.ТипыОшибок().Пропущен Тогда
