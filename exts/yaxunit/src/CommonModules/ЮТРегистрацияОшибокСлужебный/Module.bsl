...
Функция КонтекстДеталиОшибки() Экспорт
	
	Возврат ЮТКонтекстСлужебный.ЗначениеКонтекста(ИмяКонтекстаДеталиОшибки());
	
КонецФункции

Функция КлиентСерверныйКонтекстОшибки()
	
	Контекст = ЮТКонтекстСлужебный.ЗначениеКонтекста(ИмяКонтекстаОшибки());
#Если НЕ Сервер Тогда
	
	СерверныйКонтекст = ЮТКонтекстСлужебный.ЗначениеКонтекста(ИмяКонтекстаОшибки(), Истина);
	Если СерверныйКонтекст <> Неопределено Тогда
		
		Для Каждого Элемент Из СерверныйКонтекст.Пояснения Цикл
			ДобавитьПояснениеОшибки(Элемент);
		КонецЦикла;
		
		Для Каждого Элемент Из СерверныйКонтекст.ДополнительныеДанные Цикл
			ДобавитьДополнительныеДанныеОшибки(Элемент.Описание, Элемент.Данные);
		КонецЦикла;
...
Процедура ДобавитьСообщенияПользователю(Контекст)
...
#Если Сервер ИЛИ ТолстыйКлиентОбычноеПриложение ИЛИ ТолстыйКлиентУправляемоеПриложение Тогда
	...
	Если Сообщения.Количество() Тогда
		ДобавитьДополнительныеДанныеОшибки("Сообщения пользователю", Сообщения);
	КонецЕсли;
#КонецЕсли
	
КонецПроцедуры

Функция ТекстДополнительныеДанные(ДополнительныеДанные)
	Возврат СтрШаблон("%1:
		|	%2", ДополнительныеДанные.Описание, СтрСоединить(ДополнительныеДанные.Данные, Символы.ПС + Символы.Таб));
КонецФункции
