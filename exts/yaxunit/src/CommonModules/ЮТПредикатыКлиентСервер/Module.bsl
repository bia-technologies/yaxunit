...
Функция НаборПредикатов(Знач Предикаты) Экспорт
	...
	Если ТипЗнч(Предикаты) = Тип("ОбщийМодуль") И Предикаты = ЮТПредикаты Тогда
		Предикаты = ЮТПредикаты.Получить();
	ИначеЕсли ТипЗнч(Предикаты) = Тип("Структура") Тогда
		Предикаты = ЮТОбщий.ЗначениеВМассиве(Предикаты);
	КонецЕсли;
...
Функция ШаблонВыражения(Знач Выражение) Экспорт
	...
	ИначеЕсли Выражение = Выражения.Содержит Тогда
		Шаблон = "содержит `%1`";
	Иначе
		ВызватьИсключение "Не описан шаблон сообщения для выражения предаката " + Выражение;
	КонецЕсли;
