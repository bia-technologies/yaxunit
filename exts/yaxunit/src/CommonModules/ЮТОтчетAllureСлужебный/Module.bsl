...
Функция ИмяМодуляПоСхеме(ИмяМодуля)
	...
	Суффикс = Части[Части.Количество() - 1];
	// ТипМодуляПоСуффиксу возвращает исходную строку, если суффикс не распознан,
	// поэтому отличие результата от исходного значения означает известный суффикс типа модуля
	ТипМодуля = ТипМодуляПоСуффиксу(Суффикс);
	ЕстьСуффикс = (ТипМодуля <> Суффикс);

	Префикс = Части[0];
	Части.Удалить(0);
	Если ЕстьСуффикс Тогда
		Части.Удалить(Части.Количество() - 1);
	КонецЕсли;

	// После отсечения префикса и суффикса должно остаться имя объекта
	Если Части.Количество() = 0 Тогда
		Описание.epic = ИмяМодуля;
		Возврат Описание;
	КонецЕсли;

	Описание.epic = ТипОбъектаПоПрефиксу(Префикс);
	Описание.feature = СтрСоединить(Части, "_");

	Если ЕстьСуффикс Тогда
		Описание.story = ТипМодуля;
	КонецЕсли;
