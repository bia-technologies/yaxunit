...
Процедура ЗарегистрироватьТестыФормы(ОписаниеОбъектаМетаданных, ОписаниеФормы)
	...
	Если ОписаниеФормы.ЭтоФормаОбъекта И ОписаниеФормы.ЭтоФормаГруппы Тогда
		...
		ЗарегистрироватьТест(ОписаниеОбъектаМетаданных, "ФормаГруппы", "Новый", "новой группы");
		
	ИначеЕсли ОписаниеФормы.ЭтоФормаОбъекта Тогда
		
		ЗарегистрироватьТест(ОписаниеОбъектаМетаданных, "ФормаОбъекта", "Новый", "нового объекта");
		ЗарегистрироватьТест(ОписаниеОбъектаМетаданных, "ФормаОбъекта", "Существующий", "существующего объекта");
		ЗарегистрироватьТест(ОписаниеОбъектаМетаданных, "ФормаОбъекта", "Скопированный", "скопированного объекта");
...
Процедура ЗарегистрироватьТест(ОписаниеОбъектаМетаданных, ИмяФормы, Суффикс, Представление)
	
	Идентификатор = СтрШаблон("%1.%2.%3.%4", ОписаниеОбъектаМетаданных.Тип, ОписаниеОбъектаМетаданных.Имя, ИмяФормы, Суффикс);
	
	Если ЮТДымовыеТесты.ИсключитьТест(Идентификатор) Тогда
		Возврат;
	КонецЕсли;
	
	ИмяМетода = СтрШаблон("Открыть%1_%2", ИмяФормы, Суффикс);
	ПредставлениеТеста = СтрШаблон("Открытие формы %1: %2.%3", Представление, ОписаниеОбъектаМетаданных.Тип, ОписаниеОбъектаМетаданных.Имя);
	
	ЮТТесты.ДобавитьКлиентскийТест(ИмяМетода)
		.СПараметрами(ОписаниеОбъектаМетаданных)
...
Процедура ОткрытьФормаГруппы(ОбъектМетаданных) Экспорт
