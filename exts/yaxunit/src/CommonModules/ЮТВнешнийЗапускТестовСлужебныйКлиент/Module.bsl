...
Процедура ПодключениеВебСокетДо8_3_27(АдресСервера)
	...
	//	Оповещение("Не удалось подключится");
	//	Возврат;
	//КонецЕсли;
...
Процедура ПодключениеВебСокет8_3_27(АдресСервера)
	...
	МенеджерСоединений.ОткрытьСоединение("yaxunit-rpc", АдресСервера, Обработчики);
...
Процедура ОтправитьСообщение(Соединение, ТипСообщения, Данные)
	
	Сообщение = НовоеСообщение(ТипСообщения, Данные);
...
Процедура ЗапуститьТест(Сообщение, Соединение)
	...
	Если ЗначениеЗаполнено(Данные.method) И СтрСравнить("ИсполняемыеСценарии", Данные.method) <> 0 Тогда
		ИменаМетодов = ЮТКоллекции.ЗначениеВМассиве(ОписаниеМодуля.ИмяМодуля + "." + Данные.method);
	Иначе
