...
Функция ВызватьСервернуюОбработкуСобытияИсполненияТестов(ИмяСобытия, ОписаниеСобытия) Экспорт
	
	Параметры = Новый Массив(3);
	
	Если ОписаниеСобытия.Модуль <> Неопределено Тогда
		Параметры[0] = ЮТИсполнительСлужебныйКлиентСервер.ИдентификаторТестовогоМодуля(ОписаниеСобытия.Модуль);
	КонецЕсли;
	
	Если ОписаниеСобытия.Набор <> Неопределено Тогда
		Параметры[1] = ЮТИсполнительСлужебныйКлиентСервер.ИдентификаторТестовогоНабора(ОписаниеСобытия.Модуль, ОписаниеСобытия.Набор);
	КонецЕсли;
	
	Если ОписаниеСобытия.Тест <> Неопределено Тогда
		Параметры[2]= ЮТИсполнительСлужебныйКлиентСервер.ИдентификаторТеста(ОписаниеСобытия.Модуль, ОписаниеСобытия.Набор, ОписаниеСобытия.Тест);
	КонецЕсли;
	
	Возврат ВызватьПодключаемыйОбработчикСобытия(ИмяСобытия, "ИсполнениеТестов", Параметры);
