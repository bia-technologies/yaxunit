...
Функция ВызватьПодключаемыйОбработчикСобытия(ИмяСобытия, Подсистема, ПараметрыСобытия)
	...
	Для Каждого ИмяМодуля Из ОбработчикиСобытий Цикл
		...
		Если ЮТМетодыСлужебный.МетодМодуляСуществует(ИмяМодуля, ИмяСобытия) Тогда
			...
			Ошибка = ЮТМетодыСлужебный.ВыполнитьМетодКонфигурации(ИмяМодуля, ИмяСобытия, ПараметрыСобытия);
...
Процедура ВызватьОбработчикТестовогоМодуля(Знач ИмяСобытия, ОписаниеСобытия)
	...
	Если ЮТМетодыСлужебный.МетодМодуляСуществует(ИмяМодуля, ИмяСобытия) Тогда
		...
		ЮТЛогирование.Отладка(СтрШаблон("Обработка события `%1` - `%2.%3`", ИмяСобытия, ИмяМодуля, ИмяСобытия));
		Ошибка = ЮТМетодыСлужебный.ВыполнитьМетодКонфигурации(ИмяМодуля, ИмяСобытия);
	...
	ИначеЕсли ОбработчикСобытияПереопределен Тогда
		...
		ТекстИсключения = СтрШаблон("Не найден обработчик тестового модуля %1.%2", ИмяМодуля, ИмяСобытия);
