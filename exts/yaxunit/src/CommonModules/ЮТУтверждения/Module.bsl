...
Функция СоответствуетПредикату(Предикат, ОписаниеПроверки = Неопределено) Экспорт
	...
	Если НЕ РезультатПроверки.Успешно Тогда
		ОбработатьРезультаПроверки(РезультатПроверки);
	КонецЕсли;
...
Процедура ПроверитьПредикат(Выражение, ОжидаемоеЗначение, ОписаниеПроверки, ПараметрыСравнения = Неопределено)
	...
	Контекст = Контекст();
	Предикат = ЮТФабрика.ВыражениеПредиката(Выражение, Контекст.ИмяСвойства, ОжидаемоеЗначение);
	
	ПолноеОписаниеПроверки = ЮТОбщий.ДобавитьСтроку(Контекст.ПрефиксОшибки, ОписаниеПроверки, " ");
	
	ПараметрыСообщенийОбОшибке = ЮТСравнениеКлиентСервер.ПараметрыСообщенийОбОшибке(ПолноеОписаниеПроверки);
	Результат = ЮТСравнениеКлиентСервер.ПроверитьПредикаты(Контекст.ПроверяемоеЗначение, Предикат, ПараметрыСообщенийОбОшибке, ПараметрыСравнения);
