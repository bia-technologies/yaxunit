...
Функция ВыполнитьМетодМодуля(Модуль, ИмяМетода, ПараметрыМетода)
	...
	Если ВызоваМетодаОбъекта Тогда
		Ошибка = ЮТМетодыСлужебный.ВыполнитьМетодОбъекта(Модуль, ИмяМетода, ПараметрыМетода);
	Иначе
		Ошибка = ЮТМетодыСлужебный.ВыполнитьМетодКонфигурации(Модуль, ИмяМетода, ПараметрыМетода);
	КонецЕсли;
