...
Процедура ЭтоИстина() Экспорт
	...
	Исключение
		Ошибка = ИнформацияОбОшибке();
	КонецПопытки;
	ПроверитьОшибкуУтверждения(Ошибка, "Ожидали, что проверяемое значение `ОбщийМодуль` является истиной, но это не так");
...
Процедура НеИмеетТип() Экспорт
	...
	Исключение
		Ошибка = ИнформацияОбОшибке();
	КонецПопытки;
	ПроверитьОшибкуУтверждения(Ошибка, "Ожидали, что проверяемое значение `` не имеет тип `Неопределено`, но это не так");
...
Процедура ПроверитьОшибкуУтверждения(ИнформацияОбОшибке, ОжидаемоеОписание)
	...
	ИначеЕсли НЕ СтрНачинаетсяС(ИнформацияОбОшибке.Описание, "[Failed]") Тогда
		...
		ЮТРегистрацияОшибок.УстановитьДанныеОшибкиСравнения(ОжидаемоеОписание, ИнформацияОбОшибке.Описание);
