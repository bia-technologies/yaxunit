...
Функция БлокиСтатистики(Статистика)
	...
	Для Инд = 0 По Блоки.ВГраница() Цикл
		...
		ТекстКоличество = Строка(Блоки[Инд].Количество);
		ШиринаБлока = Макс(Высота * 2, СтрДлина(ТекстКоличество) * 8 + 12);
		
		Текст = Текст + СтрШаблон(
			"	<rect x=""%1"" y=""2"" width=""%2"" height=""%3"" rx=""4"" ry=""4"" style=""fill:none;stroke:#%4;stroke-width:2""/>
...
Процедура ПослеЗагрузкиТестов(Результат, ПараметрыЗапуска) Экспорт
	...
	Для Каждого ТестовыйМодуль Из ИсполняемыеТестовыеМодули Цикл
		...
		Для Каждого Набор Из ТестовыйМодуль.НаборыТестов Цикл
			...
			ТегиНабора = ЮТСтроки.УникальныеСтроки(ЮТКоллекции.ОбъединитьМассивы(ТестовыйМодуль.Теги, Набор.Теги));
			...
			СтрокаНабора.Теги = СтрСоединить(ТегиНабора, ", ");
