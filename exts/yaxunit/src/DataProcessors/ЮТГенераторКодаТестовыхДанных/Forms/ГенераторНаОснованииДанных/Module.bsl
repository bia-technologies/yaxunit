...
Процедура НастроитьСтраницуНастроек()
	...
	ЕстьНастойкиКодогенерации = ЮТГенераторКода.ЕстьНастойкиКодогенерации();
	...
	Если НЕ ЕстьНастойкиКодогенерации Тогда
...
Процедура СформироватьТекстНаСервере()
	...
	ПараметрыГенерацииКода.ИспользоватьНастройкуПостоянныеДанные = ЕстьНастойкиКодогенерации И НастройкиРежимУстановкиРеквизитов = Режимы.Авто;
	ПараметрыГенерацииКода.ФорматИменованияПеременных = НастройкиФорматИменованияПеременных;
	ПараметрыГенерацииКода.ИгнорироватьКодИнициализацииПоТипам = НЕ ЕстьНастойкиКодогенерации ИЛИ НастройкиИгнорироватьКодИнициализацииПоТипам;
...
Процедура ОбновитьОписаниеРежимаУстановкиРеквизитов()
	...
	ИначеЕсли НастройкиРежимУстановкиРеквизитов = Режимы.Поиск Тогда
		Описание = "Испльзуются существующие в базе данные. Выполняется поиск по номеру, коду или наименованию";
	ИначеЕсли НастройкиРежимУстановкиРеквизитов = Режимы.Создание Тогда
		Описание = "Испльзуются программно создананные данные";
	Иначе
...
Процедура ОбновитьОписаниеФорматаИменованияПеременных()
	...
	ОписаниеСуффикс = "Дополнительно, если уже имеется переменная с таким же именем, до будет добавлен числовой суффикс: 'Товары', 'Товары2', 'Товары3'";
	...
	ИначеЕсли НастройкиФорматИменованияПеременных = Формат.ПоПредставлению Тогда
		...
		|Если представление начинается с числа, то будет добавлен сивол '_'.
