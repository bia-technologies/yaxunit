...
Процедура ДобавитьРеквизит(МассивРеквизитов, Реквизит, ЭтоСтандартныйРеквизит = Ложь)
	...
	НовРеквизит.Вставить("ТипыРеквизитов", ПолучитьТипыСтрокой(Реквизит.Тип));
...
Функция ДанныеПоОбъектуМетаданных(СсылкаНаОбъект)
	...
	Для Каждого Реквизита Из ОбъектМетаданных.СтандартныеРеквизиты Цикл
		
		ДобавитьРеквизит(ПараметрыОбъекта.Реквизиты, Реквизита, Истина);
		
	КонецЦикла;
	
	// По реквизитам
	Для Каждого Реквизита Из ОбъектМетаданных.Реквизиты Цикл
		
		ДобавитьРеквизит(ПараметрыОбъекта.Реквизиты, Реквизита);
...
Функция СформироватьКодПоСсылке(ПараметрыФормирования, ПараметрыОбъектаМетаданных, ЗначениеСсылка)
	...
	ТипКода = ПараметрыФормирования.ТипКода;
	...
	Для Каждого ТекРеквизит Из Реквизиты Цикл
		...
		Если ТекРеквизит.СтандартныйРеквизит
			И ТекРеквизит.Данные <> "Наименование"
		...
		// * Вызов универсальной функции заполнения кода.
		ТекстКодаРеквизита = ЗаполнитьЭлементКодаПоСсылке("ЗаполнитьКодРеквизитаПоСсылке", ПараметрыФормирования, ПараметрыОбъектаМетаданных, ЗначениеСсылка, ТекРеквизит);
	...
	Для Каждого ТекТабличнаяЧасть Из ТабличныеЧасти Цикл
		...
		// * Вызов универсальной функции заполнения кода.
		ТекстКодаРеквизита = ЗаполнитьЭлементКодаПоСсылке("ЗаполнитьКодШапкиТаблЧастиПоСсылке", ПараметрыФормирования, ПараметрыОбъектаМетаданных, ЗначениеСсылка, ТекТабличнаяЧасть);
		...
		Для Каждого СтрокаТЧ Из ЗначениеСсылка[ТекТабличнаяЧасть.Данные] Цикл
			...
			// * Вызов универсальной функции заполнения кода.
			ТекстКодаРеквизита = ЗаполнитьЭлементКодаПоСсылке("ЗаполнитьКодШапкиНовойСтрокиТаблЧастиПоСсылке", ПараметрыФормирования, ПараметрыОбъектаМетаданных, ЗначениеСсылка, ТекТабличнаяЧасть);
...
Функция ЗаполнитьЭлементКодаПоСсылке(ИмяНачалаФункции, ПараметрыФормирования, ПараметрыОбъектаМетаданных, ЗначениеСсылка,
	...
	ЧетверыйПараметр = ?(ПараметрыРеквизитаТабличнойЧасти = Неопределено, "", ", ПараметрыРеквизитаТабличнойЧасти");
	
	ВыражениеДляВыполнения = СтрШаблон(ШаблонКодаДляВыполнения,	ИмяНачалаФункции, ПараметрыФормирования.ТипКода, ТретийПараметр, ЧетверыйПараметр);
...
Функция ПолучитьТекстЗначения(ЗначениеСсылка, ИмяРеквизита, ИмяТЧ = Неопределено, ДопИнфоКомментарий = "")
	Попытка
		...
		ИначеЕсли Перечисления.ТипВсеСсылки().СодержитТип(ТекТип) Тогда
			...
			ДопИнфоКомментарий = " // Еще варианты: " + ВариантыПеречисленийВСтроку(Метаданные.Перечисления[ИмяПеречисления].ЗначенияПеречисления, ", ", ИмяЗначенияПеречисления);
...
Функция ПолучитьТипыСтрокой(Тип, ВыводитьВсеТипы = Ложь)
