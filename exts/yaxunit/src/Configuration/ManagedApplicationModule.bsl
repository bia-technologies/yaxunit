...
Процедура ЮТПриНачалеРаботыСистемы()
	...
	ЮТПараметрыПодключенияКВнешнемуСервису = Новый Структура("АдресСервера, Ключ", "ws://localhost:46353", "test");
	ЮТВнешнийЗапускТестовСлужебныйКлиент.ПодключитсяКСерверу(ЮТПараметрыПодключенияКВнешнемуСервису);
